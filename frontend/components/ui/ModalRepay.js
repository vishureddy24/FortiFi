      ...
      {repayResult?.transactionHash ? (
        ...
        <div>
          <div className="p-6 pt-1 space-y-3">
              ...
              <div className="w-full justify-between flex items-center mt-2">
                ...
                <div className="flex items-center">
                  <p className="font-medium text-sm text-gray-400">
                    Wallet Balance: {todp(token.walletBalance.amount, 4)}
                  </p>
                  <button
                    onClick={() => {
                      setValue(max);
            ...
            {Number(value) > Number(max) && (
              <p className="text-xs font-bold text-amber-400 mt-2 animate-pulse uppercase tracking-wider">
                ⚠️ Amount exceeds maximum payable balance
              </p>
            )}
          </div>
              ...
              <div className="w-full flex items-center justify-between text-sm sm:text-md">
                ...
                <div className="flex items-center">
                  <Image
                    ...
                    alt="coinimage"
                  />
                  <p className="font-medium text-sm ml-2 text-gray-300">
          ...
          <div className="flex w-full items-center p-6 space-x-2 rounded-b">
            <button
                disabled={!!!value || promiseInProgress || Number(value) > Number(max)}
            ...
            >
              <div className="flex justify-center ">
                <LoadingSpinerComponent
                  buttonText={Number(value) > Number(max) ? "Exceeds Balance" : `Repay ${token?.name}`}
                  loadingMessage={transactionStep || `Repaying ${token?.name}`}
                />
              </div>
            </button>
          </div>
