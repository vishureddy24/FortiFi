          ...
          <div className="p-6 pt-1 space-y-3">
            ...
            {Number(value) > Number(max) && (
              <p className="text-xs font-bold text-amber-400 mt-2 animate-pulse uppercase tracking-wider">
                ⚠️ Amount exceeds maximum withdrawable balance
              </p>
            )}
            ...
            >
              <div className="flex justify-center ">
                <LoadingSpinerComponent
                  buttonText={Number(value) > Number(max) ? "Exceeds Balance" : `Withdraw ${token?.name}`}
                  loadingMessage={transactionStep || `Withdrawing ${token?.name}`}
                />
              </div>
            </button>
          </div>
